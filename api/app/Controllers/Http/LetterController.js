...
class LetterController {
  ...
  async create({ request }) {
    ...
    if (formData.letters[0].email) {
      try {
        user = await User.findOrCreate({ email: formData.letters[0].email }, {
  ...
  async update({ request }) {
    ...
    await Promise.all(updates.map(async (letter) => {
  ...
  async sign({ request }) {
    ...
    const letter = await Letter.query().where('slug', request.params.slug).where('locale', request.params.locale).first();
